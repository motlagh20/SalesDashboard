...
export default function RepresentativeDashboard({
  ...
  return (
    <div className="space-y-6 text-right dir-rtl font-sans" id="rep-dashboard">
      ...
      {/* 1. Header & Agent Information Banner */}
      <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-3" id="agent-info-card-header">
        {sandboxEnabled && currentUser?.role === 'SALES_MANAGER' && (
          <div className="bg-emerald-50/70 rounded-xl border border-emerald-100 p-3 mb-2" id="agent-selector-box">
            ...
            <select
              ...
              className="w-full bg-white border border-emerald-200 rounded-xl py-2 px-3 text-xs sm:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-sans cursor-pointer shadow-xs"
              id="agent-dropdown"
            >
              {agents.map((agent) => (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center justify-between sm:justify-start gap-2">
            <span className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 py-1.5 px-3 rounded-full font-sans font-bold flex items-center gap-1.5 shrink-0">
            ...
            {onOpenEditProfile && (
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 text-white border border-emerald-500 px-3 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1 cursor-pointer shadow-xs shrink-0"
                title="ویرایش شماره همراه و آدرس دقیق دفتر/انبار نمایندگی"
              >
                <User className="w-3.5 h-3.5 text-emerald-100" />
                <span>✏️ ویرایش پروفایل نمایندگی</span>
              </button>
            )}
          </div>
          ...
          <div className="text-right">
            <div className="flex items-center gap-2 justify-start sm:justify-end flex-wrap">
              <span className="text-base sm:text-lg font-extrabold text-slate-900">{currentAgentObj?.alias || currentAgentObj?.fullName || 'نامشخص'}</span>
              <span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 font-mono">
                کد: <strong>{currentAgentObj?.agentCode || 'AG-0000'}</strong>
              </span>
            </div>
            <div className="text-xs text-slate-600 mt-1.5 flex flex-wrap items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-150">
              <span className="flex items-center gap-1 text-slate-700 font-bold">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                <span>محدوده مجاز فعالیت:</span>
              </span>
      ...
      {/* 2. Order KPI Stats Overview Bar */}
      ...
      {/* Quick Launch Banner for Order Wizard */}
      <div className="bg-linear-to-r from-emerald-800 via-emerald-900 to-teal-950 text-white p-4 sm:p-5 rounded-2xl shadow-lg flex flex-col sm:flex-row items-center justify-between gap-3 border border-emerald-500/30" id="rep-quick-launch-banner">
        <div className="space-y-1 text-center sm:text-right">
          <h3 className="font-black text-sm sm:text-base flex items-center justify-center sm:justify-start gap-2">
            <Sparkles className="w-5 h-5 text-amber-300 animate-pulse" />
            <span>ثبت فوری سفارش جدید (فرم ۳ مرحله‌ای آسان)</span>
          </h3>
          <p className="text-xs text-emerald-100/90 font-medium">
            بدون نیاز به اسکرول — لود مستقیم فرم هوشمند ۳ گامه در تمام صفحه
          </p>
        </div>
        ...
        <button
          ...
          className="bg-amber-400 hover:bg-amber-300 text-slate-950 font-black px-5 py-2.5 rounded-xl text-xs sm:text-sm shadow-md transition-all cursor-pointer flex items-center gap-2 shrink-0 border-2 border-amber-300 active:scale-95"
          id="btn-quick-create-order"
        >
          <PlusCircle className="w-4.5 h-4.5 text-slate-950" />
          <span>🚀 باز کردن فرم ثبت سفارش</span>
        </button>
      </div>

      {/* 3. Main Dashboard View Selector Tabs */}
      <div className="bg-slate-200/70 p-1.5 rounded-2xl border border-slate-300/70 shadow-xs flex items-center justify-between gap-2" id="rep-tabs-container">
        <button
          ...
          className={`flex-1 py-3 px-4 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all cursor-pointer ${
            isCreateModalOpen
              ...
              : 'bg-white text-emerald-700 hover:bg-slate-50 border border-slate-200'
          }`}
          id="rep-tab-create"
        >
          <PlusCircle className="w-4.5 h-4.5 text-emerald-600" />
          <span>➕ ثبت سفارش جدید (مدال ۳ مرحله‌ای)</span>
        </button>
        ...
        >
          ...
          <span>پیگیری و لیست سفارشات</span>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
              <button
                ...
                className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-all cursor-pointer shrink-0 ${
                  trackingFilter === 'ALL' ? 'bg-slate-800 text-white border-slate-800 shadow-xs' : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                }`}
              >
                کل سفارشات ({agentOrders.length})
              </button>
              ...
              <button
                ...
                className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-all cursor-pointer shrink-0 ${
                  trackingFilter === 'PENDING' ? 'bg-amber-600 text-white border-amber-600 shadow-xs' : 'bg-amber-50 text-amber-800 border-amber-200 hover:bg-amber-100'
                }`}
              >
                در انتظار بررسی ({agentOrders.filter(o => o.status === 'PENDING_APPROVAL').length})
              </button>

              <button
                type="button"
                onClick={() => setTrackingFilter('APPROVED')}
                className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-all cursor-pointer shrink-0 ${
                  trackingFilter === 'APPROVED' ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs' : 'bg-indigo-50 text-indigo-800 border-indigo-200 hover:bg-indigo-100'
                }`}
              >
                تایید شده / صف کارخانه ({agentOrders.filter(o => o.status === 'APPROVED_BY_SALES' || o.status === 'SENT_TO_FACTORY' || o.status === 'VEHICLE_ASSIGNED').length})
              </button>

              <button
                type="button"
                onClick={() => setTrackingFilter('DISPATCHED')}
                className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-all cursor-pointer shrink-0 ${
                  trackingFilter === 'DISPATCHED' ? 'bg-emerald-600 text-white border-emerald-600 shadow-xs' : 'bg-emerald-50 text-emerald-800 border-emerald-200 hover:bg-emerald-100'
                }`}
              >
                بارگیری و حمل شده ({agentOrders.filter(o => o.status === 'LOADED_AND_DISPATCHED').length})
              </button>
            </div>
